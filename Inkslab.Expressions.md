...
| `Break`/`Continue` 越界 | `BreakExpression` / `ContinueExpression` 没有出现在 `LoopExpression` 内部 |
...
| `Break` | `LoopExpression` 内部 | 跳出循环 |
| `Continue` | `LoopExpression` 内部 | 跳回循环头 |
...
- `BreakExpression` / `ContinueExpression` 仅可出现在 `LoopExpression` 的 `Append` 链中。
