...
namespace Inkslab.Intercept.Proxys
{
    ...
    {
        private class ProxyVoidMethod : ProxyMethod
        {
            private static readonly Type _interceptAttribute = typeof(InterceptAttribute);
            private static readonly Type _interceptsAttribute = typeof(InterceptAttribute[]);
            ...
            static ProxyVoidMethod()
            {
                var middlewareInterceptType = typeof(MiddlewareIntercept);

                _middlewareInterceptFn = middlewareInterceptType.GetMethod("Run", new Type[] { _interceptContextType });
                _middlewareInterceptCtor = middlewareInterceptType.GetConstructor(new Type[] { _invocationType, _interceptsAttribute });
            }

            public ProxyVoidMethod(MethodInfo method, IList<CustomAttributeData> attributeDatas) : base(method, attributeDatas)
            {
            ...
            protected override MethodEmitter OverrideMethod(ClassEmitter classEmitter, FieldEmitter servicesAst, Expression instanceAst, MethodInfo methodInfo)
            {
                ...
                blockAst.Append(Assign(interceptVar, interceptAst))
                    ...
                    .Append(Call(interceptVar, _middlewareInterceptFn, contextVar));
